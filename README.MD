...
git clone https://github.com/wangxunx/lerobot.git
cd lerobot
git checkout 519_work_branch
pip install -e .
```
...
cp -r ../../lerobot_franka_teleop/lerobot_teleoperator_franka . # over-write the lerobot_teleoperator_franka, change the source path per your directory
pip install -e .
```
...
```bash
utils-joint-offsets   # check it everytime start record
```
