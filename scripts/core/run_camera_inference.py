...
class CameraInferenceConfig:
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        self.camera_rois: Dict[str, Tuple[int, int, int, int]] = {}
        for cam_key, cam_cfg in cams.items():
            roi = cam_cfg.get("roi")
            if isinstance(roi, str):
                with open(roi, encoding="utf-8") as f:
                    rd = json.load(f)
                self.camera_rois[cam_key] = (rd["x"], rd["y"], rd["w"], rd["h"])
            elif isinstance(roi, dict):
                self.camera_rois[cam_key] = (roi["x"], roi["y"], roi["w"], roi["h"])
...
def _apply_roi(
    frame_hwc: np.ndarray, roi: Tuple[int, int, int, int]
) -> np.ndarray:
    """Crop ROI (x, y, w, h) from frame and resize back to original resolution."""
    x, y, w, h = roi
    orig_h, orig_w = frame_hwc.shape[:2]
    cropped = frame_hwc[y:y+h, x:x+w]
...
def run_camera_inference(cfg: CameraInferenceConfig) -> None:
    ...
    for k, v in cfg.cameras.items():
        roi_str = ""
        if k in cfg.camera_rois:
            rx, ry, rw, rh = cfg.camera_rois[k]
            roi_str = f"  roi=({rx},{ry},{rw},{rh})"
        _say(f"    {k:40s} -> serial={v['serial']} {v['width']}x{v['height']}{roi_str}")
        ...
        with torch.no_grad():
            while cfg.max_steps is None or step < cfg.max_steps:
                ...
                for dataset_key, cam in cameras.items():
                    ...
                    if dataset_key in cfg.camera_rois:
                        frame = _apply_roi(frame, cfg.camera_rois[dataset_key])
